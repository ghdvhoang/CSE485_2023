...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 17, 2024 at 08:59 PM
...
INSERT INTO `tacgia` (`ma_tgia`, `ten_tgia`, `hinh_anh`) VALUES
(1, 'Nhacvietplus', NULL),
(2, 'Sưu tầm', NULL),
(3, 'Sandy', NULL),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_music`  AS SELECT `baiviet`.`tieude` AS `ten_bviet`, `tacgia`.`ten_tgia` AS `ten_tgia`, `theloai`.`ten_tloai` AS `ten_tloai` FROM ((`baiviet` join `tacgia` on(`baiviet`.`ma_tgia` = `tacgia`.`ma_tgia`)) join `theloai` on(`baiviet`.`ma_tloai` = `theloai`.`ma_tloai`)) ;
